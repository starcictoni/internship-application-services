...
async def generate_potvrda(request):
    ...
    if "detaljni_opis_zadatka" in data:
        ...
        for s in slice_rich_text:
            ...
            slice_of_text_after_last_word = rich_text[s.rfind(s.split()[-1])+start_postions[s] : ]
            ...
            if last_word_in_row != complete_last_word:
                ...
                if without_beginning:
                    without_beginning = without_beginning[0:location_of_last_word_in_row - start_postions[s] - remove_beginning]
    ...
    response.append({"attachment":encoded_pdf, "attachment_url":"http://0.0.0.0:8083/public/"+pdf_name})
